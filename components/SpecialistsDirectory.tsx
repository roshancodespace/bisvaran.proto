...
interface SpecialistsDirectoryProps {
    initialSpecialists: SpecialistData[];
}

export default function SpecialistsDirectory({ initialSpecialists }: SpecialistsDirectoryProps) {
    ...
    // Extract unique states for the filter based on all data
    const states = ["All Locations", ...Array.from(new Set(initialSpecialists.map(s => s.state)))].sort();

    const filteredData = initialSpecialists.filter(s => {
